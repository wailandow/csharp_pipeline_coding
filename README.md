# C# pipeline coding: LLM генерирует и автокомпилирует C#-стратегии

Задача: по текстовому описанию торговой стратегии получить рабочий
C#-класс — LLM пишет код, внешний Roslyn-раннер его компилирует и
считает метрики на исторических данных; при ошибке компиляции или
провале метрик пайплайн сам возвращает ошибку в промпт и повторяет
генерацию (до `max_attempts`), пока не получится или попытки не
закончатся. Результат — сохранённый `.cs`-файл, JSON с метриками и
полная история попыток в sqlite. Реализация — `projects/strategy_selector`
поверх универсального LangGraph-каркаса `core/`; тот же каркас
используется ещё в двух демо-сценариях (новостной агрегатор, помощник
по продажам), см. ниже.

```
задание → LLM → C# → компиляция → выполнение/оценка
                 ↑          |
                 └── ошибка ┘
```
...
core/                          базовое ядро для нескольких проектов
  ...
  llm_client.py                 клиент к LLM с fallback по эндпойнтам
projects/
  strategy_selector/            РЕАЛЬНЫЙ проект: генерация и автокомпиляция C#
    graph.py                     сборка пайплайна (step0..step4, retry-роутинг)
    db.py                        sqlite: сессии, попытки, метрики
    templates/                   .cs-шаблоны, из которых строится промпт
examples/                      демо-сборки того же каркаса, на заглушках
  news_aggregator_graph.py       новостной агрегатор — сборка из типовых узлов
  strategy_selector_graph.py     тот же сценарий что и projects/strategy_selector,
                                  но упрощённо и с заглушкой вместо Roslyn-раннера
  sales_assistant_graph.py       помощник по продажам — human-in-loop + anti-spam
```

`projects/strategy_selector` — это не пример, а рабочий проект: он
действительно вызывает LLM, пишет `.cs`-файл на диск, запускает внешний
компилятор (`compiler_command` из `config.yaml`) и читает JSON с
метриками. Три графа в `examples/` реально собираются и прогоняются
(проверено `graph.invoke(...)`), но fetch/LLM/Telegram/compile там —
заглушки: их цель — показать сборку каркаса, а не бизнес-логику.
